...
public class Assassin : PartyMember
{
    [Header("Attack Settings")]
    public int[] baseDamageMod;

    [Header("Special Settings")]
    [SerializeField] StatusToken concealentToken;
    [SerializeField] ParticleSystem concealmentParticle;
    ...
    {
        for (int ind = battleManager.enemies.Count - 1; ind >= 0; ind--)
        {
    ...
    IEnumerator ResolveSpecial(PartyMember target)
    {
        ...
        Instantiate(concealmentParticle, target.transform.position, Quaternion.identity);
        target.AddStatusEffect(concealentToken);
